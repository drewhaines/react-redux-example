...
import {setEndpointKeys} from '../actions/redux-auth/configure';
import {
  getApiUrl,
  getCurrentSettings,
  setCurrentSettings,
...
export function applyConfig({dispatch, endpoint={}, settings={}, reset=false}={}) {
  ...
  if (getCurrentSettings().initialCredentials) {
    ...
    const decodedToken = jwt_decode(savedCreds.idToken);
    let userId = decodedToken.sub;
    // verify session credentials with API
    return fetch(`${getApiUrl(currentEndpointKey)}${currentEndpoint[currentEndpointKey].tokenValidationPath}` + userId)
      .then((response) => {
