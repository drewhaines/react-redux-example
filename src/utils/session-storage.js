...
export function getEmailSignUpUrl (endpointKey) {
  return `${getApiUrl(endpointKey)}${getSessionEndpoint(endpointKey)
    .emailRegistrationPath}?config_name=${endpointKey}`;
}
...
export function getPasswordUpdateRedirectUrl(endpointKey) {
...
export function getOAuthUrl({provider, params, endpointKey}) {
  let oAuthUrl = getApiUrl(endpointKey) +
